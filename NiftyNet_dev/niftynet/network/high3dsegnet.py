...
class High3DSegNet(BaseNet):
    def __init__(self,
        ...
        self.layers = [
            ...
            {'name': 'res_1', 'n_features': 16, 'kernels': (3, 3, 3, 3), 'repeat': 1},
            {'name': 'res_2', 'n_features': 32, 'kernels': (3, 3, 3, 3), 'repeat': 1},
            {'name': 'res_3', 'n_features': 64, 'kernels': (3, 3, 3, 3), 'repeat': 1},
            {'name': 'res_4', 'n_features': 128, 'kernels': (3, 3, 3, 3), 'repeat': 1},
    ...
    def layer_op(self, images, is_training=True, layer_id=-1, **unused_kwargs):
       ...
       ### resblocks, all kernels dilated by 8
        params = self.layers[4]
        with DilatedTensor(flow, dilation_factor=8) as dilated:
            for j in range(params['repeat']):
                res_block = HighResBlock(
                    params['n_features'],
                    params['kernels'],
...
class HighResBlock(TrainableLayer):
    ...
    def layer_op(self, input_tensor, is_training):
        ...
        if self.with_res:
            for mm in range(len(output_tensors)):
                for nn in range(1,len(output_tensors)):
                    output_tensor1 = ElementwiseLayer('SUM')(output_tensors[mm],output_tensors[nn])
